...
#define ER_LIMIT_VLUE 2u // error flag
#define INDEX_VALUE 10u
#define SIZEOF_DATABUFFER 40u
#define VARIABILITY 0.2 // variability - do bien thien dong dien
/* USER CODE END PD */
...
volatile uint16_t debounce_100ms; // flag to de-bouncing
uint8_t timer_counter = 0;
volatile bool timer_cnt3 = false; // flag to send requests

bool Flag = false; // Flag used to allow data to be sent the first time
volatile bool Flag_Uart = false;

uint8_t tx_lora_bf[3];
uint8_t data_node[INDEX_VALUE][20];
...
uint8_t RelayStatus_arr_ESP[INDEX_VALUE]; // array to receive data from ESP
uint8_t RelayStatus_arr_Node[INDEX_VALUE]; // array to receive data from node
uint8_t NodeStatus_arr[INDEX_VALUE]; // array to receive data (node status)
uint8_t NodeStatus_arr_Old[INDEX_VALUE];
float irms1_arr[INDEX_VALUE]; // array to receive current value from node
...
void Default_Display()
{
	...
	for (i = 0; i <= 5; i++)
	{
		fillRect(start_x, start_y + i * row_height, col_width * 2, line_thickness, GREY15);
	}
	for (i = 0; i <= 2; i++)
	{
		fillRect(start_x + i * col_width, start_y, line_thickness, row_height * 5 + line_thickness, GREY15);
	}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if (huart->Instance == USART1)
    {
    	...
    	//Check buzzer status be received by ESP32
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1)
	{
		...
		/*--- The function is used to check for the button status change ---*/
		Check_Uart_update(&RelayStatus_arr_ESP[0], &RelayStatus_arr_Node[0]);
		if(StDisplay == DISPLAY_SCREEN && Display_Updated_Flag == true)
		{
